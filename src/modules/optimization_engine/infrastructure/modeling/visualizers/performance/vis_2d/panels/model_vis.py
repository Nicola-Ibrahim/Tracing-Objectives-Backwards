...
def add_model_visualization(
    ...
    """Plots either Ghost 3D Scatter (probabilistic) or Surface (deterministic)."""
    
    is_probabilistic = hasattr(estimator, "sample")
    ...
    if is_probabilistic:
        ...
        if out_dim > 1:
            _add_ghost_trace(fig, row, 2, x_plot, y_plot, z_y2, input_symbol, output_symbol, 2)
        ...
        _add_surface_trace(fig, row, 1, GX1, GX2, z_y1, input_symbol, output_symbol, 1)
        _add_surface_trace(fig, row, 2, GX1, GX2, z_y2, input_symbol, output_symbol, 2)
...
def _add_surface_trace(fig, row, col, x, y, z, input_symbol, output_symbol, out_idx):
    def _sym(symbol: str, idx: int) -> str:
        subscripts = {1: "\u2081", 2: "\u2082"}
        return f"{symbol}{subscripts.get(idx, idx)}"
    ...
    fig.add_trace(
        go.Surface(
            ...
            colorscale="Cividis",
            showscale=False,
            name=f"{_sym(output_symbol, out_idx)} Surface",
            contours_z=dict(
