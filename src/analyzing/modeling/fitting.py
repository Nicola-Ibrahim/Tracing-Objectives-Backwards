...
from ...utils.preprocessing.normalizers import HypercubeNormalizer
...
def main():
    ...
    model = InterpolatorModel(
        name="linear_interpolator_v1",
        model=interpolator,
        notes="Fitted on COCO dataset with sigma=0.1",
        model_type="Linear",
    )

    # Save to W&B
    model.save_to_wandb(
        project="coco-interpolators",
        description="Kriging model trained on 100 sample points",
        parameters={"kernel": "gaussian", "sigma": 0.1},
        metrics={"mse": 0.012, "r2_score": 0.87},
    )
