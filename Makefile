...
data-generate:  # Generate synthetic Pareto front data for a specified problem
	...
	$(PYTHON) -m src.cli.generating.generate_dataset --function-id $(FUNCTION_ID)
...
data-visualize:  # Visualize the generated data
	...
	$(PYTHON) -m src.cli.visualizing.visualize_dataset --dataset-name $(DATASET_NAME)
...
model-train-inverse:  # Train an inverse model (objectives -> decisions) using a train/test split
	...
	$(PYTHON) -m src.cli.training.train_inverse_model_standard --estimator $(INVERSE_TARGET_ESTIMATOR) --dataset-name $(DATASET_NAME)
...
model-train-inverse-cv:  # Train an inverse model with k-fold cross-validation
	...
	$(PYTHON) -m src.cli.training.train_inverse_model_cv --estimator $(INVERSE_TARGET_ESTIMATOR) --dataset-name $(DATASET_NAME)
...
model-train-inverse-grid:  # Run grid search + CV for an inverse model
	...
	$(PYTHON) -m src.cli.training.train_inverse_model_grid_search --estimator $(INVERSE_TARGET_ESTIMATOR) --dataset-name $(DATASET_NAME)
...
model-train-forward:  # Train a forward model (decisions -> objectives) using a train/test split
	...
	$(PYTHON) -m src.cli.training.train_forward_model standard --estimator $(FORWARD_TARGET_ESTIMATOR) --dataset-name $(DATASET_NAME)
...
assurance-calibrate-validation:  # Fit and persist assurance calibrators for decision validation
	...
	$(PYTHON) -m src.cli.evaluating.calibrate_decision_validation --dataset-name $(DATASET_NAME)
...
model-visualize-inverse:  # Visualize diagnostics for an inverse model (objectives -> decisions)
	...
	$(PYTHON) -m src.cli.visualizing.visualize_model_performance --estimator $(INVERSE_TARGET_ESTIMATOR) --mapping-direction inverse --dataset-name $(DATASET_NAME) $(if $(model_number),--model-number $(model_number),)
...
model-visualize-forward:  # Visualize diagnostics for a forward model (decisions -> objectives)
	...
	$(PYTHON) -m src.cli.visualizing.visualize_model_performance --estimator $(FORWARD_TARGET_ESTIMATOR) --mapping-direction forward --dataset-name $(DATASET_NAME) $(if $(model_number),--model-number $(model_number),)
