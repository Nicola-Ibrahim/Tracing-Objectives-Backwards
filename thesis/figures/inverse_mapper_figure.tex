\begin{tikzpicture}[
    ...
    execute at begin picture={
        ...
        \definecolor{bgFill}{RGB}{250,251,252}     % Light Background
    },
    % --- Global Styles ---
    font=\sffamily,
    >=Latex,
...
\node[anchor=north west, font=\large\bfseries, text=colModel]
      at ([xshift=4pt,yshift=-4pt]PanelL.north west) {A. Forward Problem};
...
\node[anchor=north west, font=\large\bfseries, text=colModel]
      at ([xshift=4pt,yshift=-4pt]PanelR.north west) {B. Inverse Design};
\node[anchor=north west, font=\footnotesize\itshape, text=colModel, yshift=-6mm]
...
\node[labelSpace=colObjective] at (-3.2, 2.1) {Target Space $\mathcal{Y}$};

% Decision/Candidate Space X (Right) – more right & a bit up
\draw[spaceBlob=colDecision] (3.2,-0.4) ellipse (1.8cm and 2.3cm);
\node[labelSpace=colDecision] at (3.2, 2.1) {Candidate Space $\mathcal{X}$};
