...
\section{Learning-Based Inverse Decision Mapping}\label{sec:learning_inverse}

Machine learning motivates inverse decision mapping as amortised inference: a neural network is trained offline on paired examples so that inverse queries can be answered quickly at deployment. Speed, however, does not eliminate the structural difficulty of inversion. The inverse relation may be multi-valued and unstable, which means that any method must decide how to represent ambiguity and how to reject candidates that do not satisfy the task definition. \cite{Ongie2020}

We formalise the task with a forward model that maps a decision to an outcome,
\begin{equation}
...
An inverse query specifies a target outcome $\vect{y}^\star$ and asks for decisions that achieve it approximately,
\begin{equation}
  \text{find }\vect{x}\in\calX \text{ such that } \mathcal{L}\!\big(\vect{f}(\vect{x}),\vect{y}^\star\big)\ \text{is small}.
  \label{eq:inverse_query}
\end{equation}
This reverse problem is commonly ill-posed: solutions may not exist for some targets, may not be unique, and may be sensitive to small perturbations in $\vect{y}^\star$ or in $\boldsymbol{\varepsilon}$. In decision mapping this is not incidental; many distinct decisions can produce outcomes that are indistinguishable at the resolution implied by $\mathcal{L}$ and the noise level. 

These properties separate inverse estimators into two roles. Point estimators output a single proposal. This includes deterministic neural predictors and classical regularised formulations that pick one representative from a set of plausible solutions:
\begin{equation}
  \widehat{\vect{x}}\in\arg\min_{\vect{x}\in\calX}\ \mathcal{L}\!\big(\vect{f}(\vect{x}),\vect{y}^\star\big)\;+\;\lambda R(\vect{x}).
  \label{eq:point_estimator}
\end{equation}
Point estimates are appropriate when deployment requires a single design, but they can hide meaningful ambiguity by collapsing many compatible decisions into one selection rule. \cite{Ongie2020}

Distributional estimators treat ambiguity as a modelling target by representing the inverse as a conditional distribution over decisions. In Bayesian inverse problems, this appears as a posterior
\begin{equation}
  p(\vect{x}\mid\vect{y}^\star)\ \propto\ p(\vect{y}^\star\mid\vect{x})\,p(\vect{x}),
  \label{eq:bayes_inverse}
\end{equation}
which makes uncertainty explicit and supports generating multiple plausible candidates. This perspective also clarifies the link between regularisation and probabilistic modelling: many regularised objectives correspond to posterior maximisation under an implicit prior and likelihood.

Learning operationalises this idea by fitting a conditional model $q_\theta(\vect{x}\mid\vect{y})$ from paired data, viewed as samples from an unknown joint distribution over decisions and outcomes. A common training objective is conditional maximum likelihood,
\begin{equation}
  \theta^\star \in \arg\min_\theta\ \sum_{i=1}^N -\log q_\theta(\vect{x}_i\mid\vect{y}_i),
  \label{eq:cond_mle}
\end{equation}
which aims to match the data-supported conditional relationship rather than forcing a single inverse function. This data-centric view is also standard in simulation-based inference, where samples are generated through forward models and inference targets conditionals rather than closed-form inverses. \cite{Cranmer2020,Papamakarios2021}

At deployment, the probabilistic inverse supports the thesis workflow of candidate generation with forward verification:
\begin{equation}
  \widehat{\vect{x}}^{(k)} \sim q_\theta(\vect{x}\mid\vect{y}^\star),\qquad k=1,\dots,K,
  \label{eq:candidate_sampling}
\end{equation}
followed by ranking or filtering using $\mathcal{L}(\vect{f}(\widehat{\vect{x}}^{(k)}),\vect{y}^\star)$. The specific choice of generative family can vary, but modern density estimators and generative models provide practical mechanisms for sampling diverse candidates from learned conditionals. 

This framing also highlights a key limitation that informs later evaluation: the quality of $q_\theta(\vect{x}\mid\vect{y})$ depends on coverage of the training joint distribution in the region of outcome space where queries will be posed. When $\vect{y}^\star$ falls outside the data-supported region, a learned inverse can produce plausible-looking proposals that fail forward verification, which motivates explicit coverage-aware data handling and forward-grounded selection. 



\section{Modeling Background}\label{sec:modelling_background}

Inverse exploration relies on models as its inner loop: given candidate decisions $\vect{x}$, we must anticipate outcomes $\vect{y}$, and—crucially for design—quantify how confident we are in those predictions. Two complementary perspectives are standard. A \emph{deterministic} view learns a single mapping $f:\mathcal{X}\!\to\!\mathcal{Y}$ that returns point estimates (useful for speed and direct optimization). A \emph{probabilistic} view learns conditional distributions $p(\vect{y}\mid\vect{x})$ (and, when needed, posteriors over parameters or designs), making uncertainty an explicit object of reasoning. Both sit naturally within risk-minimization and decision-theoretic principles and will be used throughout our inverse workflows~\cite{RasmussenWilliams2006,Bishop2006PRML,HTF2009,Berger1985}.
