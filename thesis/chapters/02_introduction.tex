...
Design and analysis problems in science and engineering are increasingly driven by computational models: we choose a set of controllable variables, evaluate the resulting behaviour through analysis or simulation, and then iterate until the observed performance is acceptable. In many modern settings this workflow is complicated by three factors. First, the relationship between settings and outcomes is nonlinear and high-dimensional, so local intuition is often unreliable. Second, performance is rarely captured by a single number: designers must balance multiple objectives and constraints, and the desired outcome is better described as a target pattern of metrics than as one optimal point. Third, forward evaluation can be expensive, which makes repeated trial-and-error and repeated end-to-end optimisation costly when targets change or when interactive exploration is required.
...
\begin{equation}
  \hat{g}:\calY \to \calX,
\end{equation}
that can be queried with a target $\vect{y}^{\star}$ to propose candidate decisions intended to realise that target. Modern machine learning models are well suited to this role because they can approximate complex, high-dimensional nonlinear relationships from data and provide fast proposals at query time. When the inverse relationship is inherently multi-valued, probabilistic and generative modelling ideas offer a principled way to represent families of plausible decisions and to express ambiguity arising from noise, modelling mismatch, or limited data, rather than collapsing the problem to a single design point.

\begin{figure}[h]
  \centering
  \input{figures/inverse_mapper_figure}
  \caption[Forward and Inverse Design Mapping]{Forward vs.\ inverse viewpoint. Forward mapping evaluates how decisions $\vect{x}$ produce outcomes $\vect{y}$; the inverse task seeks decisions that realise a desired outcome pattern.}
...
The inverse viewpoint becomes especially relevant in multi-criteria design, where outcomes are vectors of competing performance measures. In such settings, designers often specify preferred trade-offs in objective space and then seek feasible decisions that realise them. This naturally connects inverse querying to the geometry of the attainable objective set and, in particular, to the Pareto front that captures non-dominated trade-offs among objectives.

\begin{figure}[h]
  \centering
  \input{figures/decision_objective_mapping}
  \caption[Decision and Objective Space Mapping]{Illustration of the mapping between the decision space (design parameters $\mathbf{x}$) and the objective space (performance metrics $\mathbf{f}$). The forward problem maps design parameters to properties, while inverse design seeks decisions $\mathbf{x}$ that satisfy target requirements in objective space, including targets near the Pareto front in multi-objective settings.}
...
We consider inverse decision mapping for multi-criteria signal-processing systems. Assume access to representative examples $\mathcal{D}=\{(\vect{x}_i,\vect{y}_i)\}_{i=1}^N$ with $\vect{x}_i\in\calX$, $\vect{y}_i\in\calY$, and $\vect{y}_i=f(\vect{x}_i)$ for a forward map $f:\calX\to\calY$. The problem is to learn a data-driven rule that, given a user-specified target $\vect{y}^{\star}\in\calY$, returns one or more candidate decisions $\widehat{\vect{x}}\in\calX$ whose forward evaluations $f(\widehat{\vect{x}})$ are close to $\vect{y}^{\star}$ under task-relevant discrepancy measures in outcome space. This inverse relationship is generally ill-posed: multiple distinct decisions may correspond to similar outcomes, some targets may have no feasible preimage, and small perturbations in $\vect{y}^{\star}$ or in observed outcomes can yield large changes in inferred decisions~\cite{KabanikhinSurvey,BenningBurger2018,Stuart2010}.

The purpose of this thesis is to develop and assess a principled approach for inverse decision mapping that is transparent, reproducible, and suitable for interactive use across application domains. The approach uses machine learning models as fast proposal mechanisms for candidate decisions, while treating forward evaluation as the reference for verification and for application-aligned assessment. This combination follows a broader direction in inverse problems research that leverages data-driven models to accelerate inverse querying without abandoning forward-model grounding.
