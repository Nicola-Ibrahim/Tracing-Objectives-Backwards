...
export function TransformationForm({ metadata, params, onChange }: TransformationFormProps) {
    ...
    return (
        <div className="space-y-4">
            {Object.entries(metadata.params).map(([name, schema]) => {
                const value = params[name] !== undefined ? params[name] : schema.default;
                ...
                return (
                    <div key={name} className="space-y-2">
                        <div className="flex justify-between items-center">
                            ...
                            {schema.description && (
                                <span className="text-[10px] text-slate-400 italic">{schema.description}</span>
                            )}
                        </div>

                        {schema.type === "number" || schema.type === "float" || schema.type === "int" ? (
                            <Input
                                id={name}
                                type="number"
                                step={schema.type === "int" ? "1" : "0.1"}
                                value={value}
                                onChange={(e) => handleParamChange(name, parseFloat(e.target.value))}
                                className="bg-slate-50 border-slate-200"
                            />
                        ) : schema.type === "list" || name === "feature_range" ? (
                            <div className="grid grid-cols-2 gap-2">
                                ...
                                <Input
                                    type="number"
                                    value={Array.isArray(value) ? value[1] : 1}
                                    onChange={(e) => {
                                        const newVal = [...(Array.isArray(value) ? value : [0, 1])];
